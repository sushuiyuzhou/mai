...
template<typename T> struct IsSerializable {
...
public:
  ...
  template<typename T> void operator=(T &&t) { set(std::forward<T>(t)); }

  Node &attach(Index sym);

  template<typename... Args> Node &attach(Args... args) {
