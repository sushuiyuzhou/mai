...
public:
  ...
  template<typename T, typename... Args,
           typename = std::enable_if_t<
               std::is_constructible_v<IndexElemType, T>, void>>
  explicit Index(T i, Args... args) : _idx{} {
    this->add(i, args...);
  }

  // helper utils
  bool empty() const { return _idx.empty(); }
  ...
  Index common(Index const &other) {
    Index res{};
    auto p = std::mismatch(_idx.cbegin(), _idx.cend(), other._idx.cbegin(),
                           other._idx.cend())
                 .first;
    if (p != _idx.cend()) {
  ...
  friend Index operator+(Index const &l, Index const &r) {
    Index res(l);
    std::copy(r._idx.cbegin(), r._idx.cend(),
              std::back_inserter(res._idx));
    return std::move(res);
  }
